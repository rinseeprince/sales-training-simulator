...
export async function GET(request: NextRequest) {
  try {
    ...
    async function getMetricsManually(user: any, days: number) {
      ...
      // Get team members
      let teamQuery = supabase.from('simple_users').select('id')
    ...
    // Get pending reviews count specifically for this manager
    const { data: pendingReviews, error: pendingError } = await supabase
      .from('call_reviews')
      .select('id')
      .eq('reviewer_id', simpleUser.id)
      .eq('status', 'pending')
    ...
    const processedMetrics = {
      ...
      approvedCalls: metrics.approved_calls || 0,
